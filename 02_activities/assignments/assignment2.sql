...
WITH vendor_product_customer AS (
    ...
    FROM 
        vendor_inventory v
    CROSS JOIN 
        (SELECT DISTINCT customer_id FROM customer_purchases) c
),
vendor_revenue AS (
    SELECT 
        v.vendor_id,
        v.product_id,
        5 * v.original_price AS revenue_per_customer
    FROM 
        vendor_product_customer v
)
SELECT 
    vi.vendor_name,
    p.product_name,
    SUM(vr.revenue_per_customer) AS total_revenue
FROM 
    vendor_inventory vi
JOIN 
    vendor_revenue vr
    ON vi.vendor_id = vr.vendor_id AND vi.product_id = vr.product_id
JOIN 
    product p
    ON vi.product_id = p.product_id
GROUP BY 
    vi.vendor_name, p.product_name;
...
VALUES 
(999, 'Apple Pie - Deluxe', '12"', 3, 'unit', CURRENT_TIMESTAMP);
...
WHERE 
    product_name = 'Apple Pie - Deluxe'
    AND snapshot_timestamp < (
        SELECT MAX(snapshot_timestamp)
        FROM product_units
        WHERE product_name = 'Apple Pie - Deluxe'
    );
